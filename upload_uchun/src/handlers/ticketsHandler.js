  ...
  bot.action(/^view_ticket_(.+)$/, async (ctx) => {
    ...
    if (ticket.replies && ticket.replies.length > 0) {
      repliesText = ticket.replies.map((r, i) => {
        ...
        const icon = r.sender_name === 'Mijoz' ? '👤' : '👨‍💻';
        return `${i + 1}. [${time}] ${icon} <b>${r.sender_name}:</b>\n«${r.text}»`;
      }).join('\n\n');
    }
    ...
    // If ticket is still open, allow user to close it directly from details
    if (ticket.status !== 'resolved' && ticket.status !== 'closed') {
      const isRu = lang === 'ru';
      const closeKb = Markup.inlineKeyboard([
        [Markup.button.callback(isRu ? '✅ Закрыть обращение' : '✅ Murojaatni yopish', `user_close_ticket_${ticket.id}`)]
      ]);
      await ctx.replyWithHTML(detailText, closeKb);
    } else {
