...
function adminTicketInlineKeyboard(ticketId, lang = 'uz') {
  ...
  return Markup.inlineKeyboard([
    [
      ...
      Markup.button.callback(isRu ? '✅ Закрыть обращение' : '✅ Murojaatni yopish', `admin_status_resolved_${ticketId}`)
    ],
    [
      Markup.button.callback(isRu ? '📋 Все открытые обращения' : '📋 Barcha ochiq murojaatlar', 'list_open_tickets')
    ]
  ]);
}
